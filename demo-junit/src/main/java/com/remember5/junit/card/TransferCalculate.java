...
public class TransferCalculate {
    ...
    public static BigDecimal calculate(BaseCard card, Integer expenseCount) {
        ...
        if (card.getCardCategory() == CardCategory.AMOUNT) {
            ...
            isLastTransfer = newCumulativeAmount.compareTo(card.getArrivalAmount()) >= 0;
        } else {
            // 判断是否为最后一次
            isLastTransfer = card.getRemainingCount() - expenseCount <= 0;
            // 更新剩余核销次数
    ...
    private static BigDecimal getPlanTransferAmount(BaseCard card, Integer expenseCount) {
        BigDecimal planTransferAmount = card.getCardCategory() == CardCategory.AMOUNT ? ((AmountCard) card).getCurrentExpenseAmount() : card.getEachAmount().multiply(new BigDecimal(expenseCount));
